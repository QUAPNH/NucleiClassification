...
class TrainManager(Config):
    ...
    ####
    def _get_datagen(self, batch_size, run_mode, target_gen, nr_procs=0, phase=None,fold_idx=0):
    ...
    def run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, fold_idx=0):
        ...
        for net_name, net_info in net_info_opt.items():
            ...
            net_desc = DataParallel(net_desc, device_ids=list(range(torch.cuda.device_count())))
        ...
        pytorch_total_params = sum(p.numel() for p in net_desc.parameters())   
...
if __name__ == "__main__":
    args = docopt(__doc__, version="Net v1.0")
