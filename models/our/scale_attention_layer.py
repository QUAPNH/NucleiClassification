...
class ChannelGate(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=24, pool_types=['avg', 'max']):
    ...
    def forward(self, x):
        ...
        channel_att_sum = channel_att_sum.reshape(channel_att_sum.shape[0], 24, 24)
        avg_weight = torch.mean(channel_att_sum, dim=2).unsqueeze(2)
        avg_weight = avg_weight.expand(channel_att_sum.shape[0], 24, 24).reshape(channel_att_sum.shape[0], 576)
...
class SpatialAtten(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_att = spatial_att.expand(spatial_att.shape[0], 24, 24, spatial_att.shape[3], spatial_att.shape[4]).reshape(
                                        spatial_att.shape[0], 576, spatial_att.shape[3], spatial_att.shape[4])
...
class Scale_atten_block(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=24, pool_types=['avg', 'max'], no_spatial=False):
...
class scale_atten_convblock(nn.Module):
    def __init__(self, in_size, out_size, stride=1, downsample=None, use_cbam=True, no_spatial=False, drop_out=False):
        ...
        if use_cbam:
            self.cbam = Scale_atten_block(in_size, reduction_ratio=24, no_spatial=self.no_spatial)  # out_size
        else:
            self.cbam = None
