...
def train_step(batch_data, run_info):
    ...
    pred_dict,pre_dec_dict = model(imgs)
    ...
    criterion = CtdetLoss()
    loss_dec = criterion(batch_data, pre_dec_dict)
    ...
    track_value("loss_dec", loss_dec.cpu().item())
    loss += loss_dec
...
def valid_step(batch_data, run_info):
    ...
    with torch.no_grad():  # dont compute gradient
        pred_dict,pre_dec_dict = model(imgs_gpu)
        pred_dict = OrderedDict(
            [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict.items()]
        )
...
def infer_step(batch_data, model):
    ...
    # --------------------------------------------------------------
    with torch.no_grad():  # dont compute gradient
        pred_dict,pre_dec_dict = model(patch_imgs_gpu)
        pred_dict = OrderedDict(
            [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict.items()]
        )
        pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]
...
def viz_step_output(raw_data, nr_types=None):
    ...
    if nr_types is not None:
        ...
        aligned_shape = [list(imgs.shape), list((2, 164, 164)), list((2, 164, 164))]
    ...
    for idx in range(imgs.shape[0]):
        ...
        #for i in range(len(true_viz_list)):
            #print(true_viz_list[i].shape)
